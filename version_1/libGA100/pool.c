   ...
   Pool_Ptr    pool;
{
   ...
   if(no_variance || var <= 0.0) {
      pool->var = 0.0;
      pool->dev = 0.0;
      ga_info->converged = FALSE; /*TRUE;  modified by claudio*/
   } else {
      pool->var = var;
      pool->dev = sqrt(var);
   ...
   /* added by claudio */
   //  if((pool->chrom[pool->best_index])->fitness == ga_info->Optimum)
   //ga_info->converged = TRUE;
   /* ga_info->solution_found = TRUE; */
   /* "Optimum" and "solution_found" are variables
      added in the ga_info struct */
}
