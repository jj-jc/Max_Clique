...
============================================================================*/
/*----------------------------------------------------------------------------
| Simple mutation, invert bit -- original version
----------------------------------------------------------------------------*/
   ...
   Chrom_Ptr chrom;
{
   ...
   chrom->gene[idx] = RAND_FRAC();
}

/*----------------------------------------------------------------------------
| Swap random elements -- original version
----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------
|  Flip all elements with probability 0.5 -- our version of swap
----------------------------------------------------------------------------*/
MU_swap_new(ga_info, chrom)
   GA_Info_Ptr ga_info;
   Chrom_Ptr chrom;
{
   int idx;
   //    if (RAND_FRAC() <0.1) 
   //    {
	      //mu_build_greedy_solution(chrom);
   //    ga_info->grasp++;
   //  }
   //  else
   //    {
	      for(idx=chrom->idx_min;idx<chrom->length;idx++)
	     if(RAND_BIT())
	      chrom->gene[idx] = chrom->gene[idx] ? 0 : 1;
	      //    }
}
