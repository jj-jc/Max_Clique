...
/*----------------------------------------------------------------------------
  | Uniform crossover OUR VERSION!!!
|
| Each allele is copied from a parent based on a random flip of a fair coin
----------------------------------------------------------------------------*/
X_uniform_old(ga_info, parent_1, parent_2, child_1, child_2)
   GA_Info_Ptr ga_info;
   Chrom_Ptr  parent_1, parent_2;
   Chrom_Ptr  child_1, child_2;
...
{  int T[MAX_VARS];
  ...
  for (i= 0; i < parent_1->length; i++) T[i]=0;
   ...
   for(i = 0; i < parent_1->length; i++) {
      if (parent_1->gene[i]==parent_2->gene[i]) 
     {
       child_1->gene[i] = parent_1->gene[i];
         child_2->gene[i] = parent_2->gene[i];
     }
 ...
 if(RAND_BIT()) {
      ...
      */
      //    mu_build_greedy_solution(child_2);
      //mu_build_greedy_solution(child_1);
